...
void scanLibrary(LibraryMirror lib) {
  ...
  var externals = lib.libraryDependencies.where((it) => it.isImport && it.metadata.any((it) => it.reflectee is ExternalGroup));
  
  for (var external in externals) {
    ExternalGroup g = external.metadata.firstWhere((it) => it.reflectee is ExternalGroup).reflectee;
...
List<ClassAnnotation> findClassesAnnotation(Type type, {LibraryMirror lib}) => (lib == null ? currentMirrorSystem().isolate.rootLibrary : lib).declarations.values.where((it) => it is ClassMirror && it.metadata.any((a) => reflectType(type).isAssignableTo(a.type))).map((it) {
...
List<FunctionAnnotation> findFunctionAnnotations(Type type, {InstanceMirror instance, LibraryMirror lib}) {
  var l = (lib == null ? currentMirrorSystem().isolate.rootLibrary : lib);
  var result = [];
  var t = reflectType(type);
  var decl = instance != null ? instance.type.declarations.values : l.declarations.values;
